...
| 收集箱 | 自定义调查表单、草稿与发布、免登录填写、文件收集、回收记录与 CSV 导出 |
...
```bash
cd server && npm test    # 算法单测 + API 集成测试 + 越权隔离测试
cd web && npm test       # 请求层、token 刷新与匿名调查接口
```
...
- **公开调查**：教师在收集箱配置字段并发布，分享 `/s/<随机标识>` 即可免登录填写。
  答卷与附件仅发布者可查看，不做离线缓存；编辑后保留旧答卷的字段快照。
  详见 [`docs/SURVEYS.md`](docs/SURVEYS.md)。
